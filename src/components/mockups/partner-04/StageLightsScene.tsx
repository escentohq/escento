...
function LightBeam({ color, position, rotation, intensity, speed }: any) {
...
function Scene() {
  ...
  return (
    <group ref={groupRef}>
      {/* Background ambient gradient spheres to simulate light bleeding */}
      <Float speed={2} rotationIntensity={0} floatIntensity={2}>
        <mesh position={[-viewport.width / 4, viewport.height / 4, -10]}>
          <sphereGeometry args={[5, 32, 32]} />
          <meshBasicMaterial color="#0055FF" transparent opacity={0.15} blending={THREE.AdditiveBlending} depthWrite={false} />
        </mesh>
      </Float>
      <Float speed={1.5} rotationIntensity={0} floatIntensity={2.5}>
        <mesh position={[viewport.width / 4, -viewport.height / 4, -15]}>
          <sphereGeometry args={[6, 32, 32]} />
          <meshBasicMaterial color="#FF3366" transparent opacity={0.12} blending={THREE.AdditiveBlending} depthWrite={false} />
        </mesh>
      </Float>
      <Float speed={2.5} rotationIntensity={0} floatIntensity={1.5}>
        <mesh position={[0, 0, -20]}>
          <sphereGeometry args={[8, 32, 32]} />
          <meshBasicMaterial color="#FFB000" transparent opacity={0.1} blending={THREE.AdditiveBlending} depthWrite={false} />
        </mesh>
      </Float>
...
export function StageLightsScene() {
  return (
    <div className="absolute inset-0 z-0 pointer-events-none bg-[#FAFAFA] overflow-hidden">

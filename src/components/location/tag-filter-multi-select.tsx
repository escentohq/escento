  ...
  return (
    <div className="text-sm font-bold text-[#0F172A]">
      ...
      <div className="relative mt-2">
        <div className=" border border-[#E2E8F0] bg-white shadow-sm focus-within:border-[#0055FF] focus-within:ring-2 focus-within:ring-[#0055FF]/20">
          <input
            id={id}
            value={query}
            onChange={(event) => {
            ...
            className="w-full  border-0 bg-transparent px-4 py-3 text-sm font-medium text-[#0F172A] outline-none placeholder:text-[#94A3B8]"
            autoComplete="off"
          />
        </div>
          ...
          <div className="absolute z-30 mt-2 max-h-64 w-full overflow-auto  border border-[#E2E8F0] bg-white p-2 shadow-xl">
            {suggestions.map((option) => {
              ...
              return (
                <button
                  ...
                  className="block w-full  px-3 py-2 text-left text-sm font-bold text-[#0F172A] transition-colors hover:bg-[#F8FAFC] disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            {showFallback ? (
              <button
                ...
                className="block w-full  px-3 py-2 text-left text-sm font-bold text-[#0055FF] transition-colors hover:bg-[#F8FAFC]"
              >
      ...
      {selectedValues.length ? (
        <div className="mt-2 flex flex-wrap gap-2">
          {selectedValues.map((value) => (
            <span key={value} className="inline-flex items-center gap-1.5  bg-[#F8FAFC] px-3 py-1 text-xs font-black text-[#0F172A]">
              {value}
              <button
                type="button"
                onClick={() => removeValue(value)}
                className=" text-[#64748B] transition-colors hover:text-[#FF3366] focus-visible:outline-2 focus-visible:outline-[#0055FF]"

...
- [ ] Any schema change is a file in `supabase/migrations/`, added to `expected_migrations` in `supabase/parity_check.sql`. `deploy-production.yml` applies and verifies it automatically on merge — see [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md).

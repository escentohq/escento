...
Apply schema changes as a migration file. A dashboard or MCP edit is allowed for exploration but must be exported back into a migration before the change ships. `docs/DEPLOYMENT.md` is the release runbook: production deploys code only, so a migration reaches hosted Supabase by a deliberate human step, and `supabase/parity_check.sql` is the read-only gate that proves hosted matches `supabase/migrations/`. `.github/workflows/schema-drift.yml` would automate that check but is `disabled_manually` and has never run — it needs a `SUPABASE_DB_URL` repo secret that does not exist.

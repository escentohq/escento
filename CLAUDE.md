...
```ts
{ user: { id, email, role, name, image } }  // id = the Supabase auth user id, reused as the app_user uuid id
```
...
- ID types are split. `app_user`, `musician_profile`, and `gig` use Postgres-native `uuid` with a `gen_random_uuid()` default; the messaging tables use `TEXT` ids generated via `crypto.randomUUID()` in application code before insert.
...
Apply schema changes as a migration file. A dashboard or MCP edit is allowed for exploration but must be exported back into a migration before the change ships. `docs/DEPLOYMENT.md` is the release runbook: `deploy-production.yml`'s `migrate` job applies `supabase/migrations/` to hosted with `supabase db push` and runs `supabase/parity_check.sql` before `deploy` is allowed to run — a broken or drifted database blocks the release rather than shipping ahead of it. `schema-drift.yml` runs the same check daily.

RLS **is** enabled on all seven content tables and is load-bearing: `SELECT … FOR UPDATE` in the transactional write RPCs makes PostgreSQL apply the UPDATE policy, which is what actually refuses a cross-owner edit. The in-function ownership assertions are the backstop for service-role callers that bypass RLS. Authenticated DML is further narrowed to explicit column grants (issue #59), so writes must name only granted columns — a bare `INSERT INTO t SELECT …` implicitly targets every column and fails with `42501` (issue #68). `updated_at` is maintained by a `BEFORE UPDATE` trigger precisely so it never needs a grant.

ID types are split, and the split is load-bearing when you write SQL: `app_user`, `musician_profile`, and `gig` use `uuid` (so `auth.uid() = user_id` compares directly), while the messaging tables (`conversations`, `conversation_participants`) use `TEXT` ids generated by `crypto.randomUUID()` in application code. All FK constraints use `ON DELETE CASCADE`.
